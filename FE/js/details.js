document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadTransactionData() {
        try {
            ...
            if (category === "All") {
                ...
                const selectedCategoryData = data.expenses?.find(expense => expense.categoryName === category);
    ...
    function updateTransactionTable(categoryData, selectedCategory) {
        ...
        if (categoryData.length === 0) {
            ...
            row.innerHTML = '<td colspan="4" style="text-align: center; color: #666;">No transactions found for this category</td>';
        ...
        if (selectedCategory === "All" && categoryData.length > 1) {
            ...
            summaryRow.style.borderTop = '2px solid #E2001A';
    ...
    function updateBalanceDisplay(data, selectedCategory) {
        if (selectedCategory === "All") {
            ...
            balanceText.innerHTML = `
                <div style="color: #E2001A; font-weight: bold; font-size: 1.2em;">Overall Summary</div>
            ...
            const categoryInfo = data.expenses?.find(expense => expense.categoryName === selectedCategory);
            if (categoryInfo) {
                balanceText.innerHTML = `
                    <div style="color: #E2001A; font-weight: bold; font-size: 1.2em;">${selectedCategory}</div>
                    <div>Total Spent: $${categoryInfo.totalAmount.toFixed(2)}</div>
                    <div>Percentage of Total: ${categoryInfo.percentageAmount.toFixed(2)}%</div>
                `;
            } else {
                balanceText.innerHTML = `
                    <div style="color: #E2001A; font-weight: bold;">No data found for ${selectedCategory}</div>
                `;
            }
        }
    }
    ...
    function showLoading() {
        ...
        balanceText.innerHTML = '<div style="text-align: center;">Loading balance...</div>';
    }

    function showError(message) {
        const tbody = document.querySelector('.table-placeholder table tbody');
        if (tbody) {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: #E2001A;">${message}</td></tr>`;
        }
        
        balanceText.innerHTML = `<div style="color: #E2001A;">${message}</div>`;
    }
    ...
    async function loadAndRenderBalance(data = null) {
        try {
            ...
            displayData.forEach(item => {
                ...
                const percent = data.totalExpenses > 0 ? (item.totalAmount / data.totalExpenses) * 100 : 0;
